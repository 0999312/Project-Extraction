...
`PlayerHUD` 场景（`scenes/game_scene/player_hud.tscn`）由 `DemoGameRuntime._setup_player_hud()` 在运行时加载。HUD 是一个 `Control` 节点，通过 `UIManager.add_overlay()` 注册为 `UILayer.SCENE`（层级 0）上的持久覆盖层。

- **加载方式**：`load("res://scenes/game_scene/player_hud.tscn")` → `instantiate()` → `UIManager.add_overlay(hud_id, hud, UILayer.SCENE)`
- **物品栏绑定**：`PlayerHUD.bind_inventory(grid)` 将 HUD 连接到玩家的 `GridInventory`（默认为背包网格）
- **默认选中**：快捷栏第 0 格默认选中（`active_hotbar_index = 0`）
- **UIManager 集成**：HUD 是覆盖层，不是栈中的面板。无论其他面板打开/关闭状态如何，它始终保持可见。

## 3. 快捷栏设计

### 3.1 视觉设计（StyleBoxFlat — 无贴图）

| 属性 | 普通格子 | 选中格子 |
|------|---------|---------|
| 尺寸 | 56 × 56 px | 56 × 56 px |
| 渲染方式 | `PanelContainer` + `StyleBoxFlat` | `PanelContainer` + `StyleBoxFlat` |
| 背景色 | 黑色，透明度 = 64 | 绿色 (0, 1, 0)，透明度 = 64 |
| 边框 | 6 px，纯黑色 | 6 px，纯黑色 |
| 圆角半径 | 8 px | 8 px |

每个格子是一个 `PanelContainer`，使用 `StyleBoxFlat` 主题覆盖。**不使用任何贴图/材质资源**。所有格子始终保持相同的正方形尺寸，选中格只通过半透明绿色填充来突出显示。
...
2. `_update_hotbar_selection()` 刷新视觉状态（切换 StyleBoxFlat）
...
物品栏菜单通过 GUIDE 动作 `pe_inventory` 切换，从而支持按键重映射。
...
| 路径 | 类型 | 用途 |
|------|------|------|
| `scripts/game/ui/player_hud.gd` | 脚本 | HUD + 快捷栏逻辑（StyleBoxFlat） |
| `scenes/game_scene/player_hud.tscn` | 场景 | HUD 场景布局 |
| `scripts/game/gameplay/demo_game_runtime.gd` | 脚本 | 游戏运行时（加载 HUD，创建装备） |
| `scripts/game/input/player_input_context.gd` | 脚本 | GUIDE 输入绑定 |
| `scripts/game/components/gameplay/equipment_state.gd` | 脚本 | 装备数据模型 |
