...
物品栏系统是 **俄罗斯方块式网格物品栏（Tetris-style grid inventory）**，物品根据其 `size_w × size_h` 占据矩形格子区域。玩家可以 **拖拽** 物品进行放置和移动。系统现在根据**装备生成不同的网格** — 每个容器装备（背包、战术弹挂）拥有各自独立的网格。系统还包括 **快捷栏**（快速访问槽位）和 **手持物品** 指示。
...
- **无效放置回退**：如果目标位置无效，物品会优先返回原始位置；若原位也不可用，则网格会尝试自动放入第一个有效位置。
