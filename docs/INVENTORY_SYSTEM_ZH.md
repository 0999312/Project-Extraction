...
- 显示所有装备类别的占位槽：主武器、副武器、近战武器、头盔、耳机、护甲、背包、弹挂。
- 目前仅显示标签（尚无拖拽交互 — 占位符 UI）。
...
- 底部显示 9 个快捷栏槽位。
- 快捷栏样式：**6 px 纯黑边框，8 px 圆角**，背景透明度 = 64。选中格深蓝色填充并略微放大。
- 将物品拖到快捷栏槽位上可进行绑定。
- 按数字键（1–9）选择激活槽位。
...
- `DemoGameRuntime._ready()` 创建 `EquipmentState`，默认配备背包（6×6）和战术弹挂（3×2），添加 `InventoryMenu`，绑定物品栏网格和装备状态。
- `PlayerHUD` 快捷栏显示从背包 `GridInventory.hotbar_slots` 更新。
