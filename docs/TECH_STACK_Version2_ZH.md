...
- **场景 / 节点驱动的玩法运行时**
  ...
  - 运行时实体定义与抛射物定义现在会先进入专用玩法注册表，再执行实例化。
  - 池化与区块激活仍然是主要性能手段。
...
- **ramokz/phantom-camera**
  - 跟随、平滑、约束、震动/抖动。
  - 通过摄像机桥接器由模拟事件控制。
  - 当前玩法运行时会在玩家与受距离约束的镜头瞄准目标之间动态切换跟随目标：
    - ADS 瞄准时：镜头跟随 `CameraAimTarget`（位置受武器 ADS 距离上限约束）；准星自由跟随鼠标
    - 腰射 / 放松状态：镜头跟随玩家
  - 跟随目标切换的平滑时长由武器瞄准时间参数（`CombatState.aim_transition_sec`）控制。
...
2. **玩法逻辑层**
   ...
   - 当前运行时脚本统一整理在 `scripts/game/` 下，而不是已经移除的旧 ECS 时代目录命名。
   - 当前运行时命名改为 actor / state / runtime 语义，不再使用历史 `C_` / `S_` / `e_` 前缀。
   - `EntityCatalog` / `EntityRegistry` 与 `ProjectileCatalog` / `ProjectileRegistry` 负责角色和抛射物的注册表化实例化。
3. **表现层（Godot 节点）**
...
- 人类敌人可继续复用 `HumanActor` 的手部瞄准骨架；非人类敌人保留全身朝向瞄准，同时遵守同一套生物角色基类契约。
