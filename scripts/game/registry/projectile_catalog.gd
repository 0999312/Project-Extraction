...
static func ensure_registry() -> void:
	...
	if registry == null:
		LocalizedText.error("logs.projectile_catalog.registry_unresolved")
		return
...
static func _load_projectiles_from_json(registry: ProjectileRegistry) -> void:
	if not FileAccess.file_exists(PROJECTILES_JSON_PATH):
		LocalizedText.warn("logs.projectile_catalog.json_missing", [PROJECTILES_JSON_PATH])
		return
	var file := FileAccess.open(PROJECTILES_JSON_PATH, FileAccess.READ)
	if file == null:
		LocalizedText.warn("logs.projectile_catalog.json_open_failed", [PROJECTILES_JSON_PATH])
		return
	var json := JSON.new()
	var err := json.parse(file.get_as_text())
	file.close()
	if err != OK:
		LocalizedText.error("logs.projectile_catalog.json_parse_failed", [PROJECTILES_JSON_PATH, json.get_error_message()])
		return
	var data: Variant = json.data
	if not (data is Dictionary):
		LocalizedText.error("logs.projectile_catalog.json_root_invalid")
		return
...
static func instantiate_projectile(projectile_id: String, overrides: Dictionary = {}) -> Projectile:
	...
	if script_path.is_empty() or not ResourceLoader.exists(script_path):
		LocalizedText.error("logs.projectile_catalog.script_missing", [projectile_id, script_path])
		return null
	var script_resource := ResourceLoader.load(script_path)
	if not (script_resource is GDScript):
		LocalizedText.error("logs.projectile_catalog.script_invalid", [script_path])
		return null
	var projectile : Projectile = (script_resource as GDScript).new()
	if not (projectile is Projectile):
		LocalizedText.error("logs.projectile_catalog.script_instance_invalid", [script_path])
		return null
